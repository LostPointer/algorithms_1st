...
void MinHeap<T>::ShiftDown(int index){
  if (2 * index + 1 > data_.size() - 1 || index > data_.size()) return;
  else{
    if (data_[index] == data_[2 * index + 1] && data_[2 * index + 1] == data_[2 * index + 2]) return;
    else if (data_[index] > data_[2 * index + 1] && data_[2 * index + 1] <= data_[2 * index + 2]){
      std::swap(data_[2 * index + 1], data_[index]);
      ShiftDown(2 * index + 1);
    }
    else if (data_[index] > data_[2 * index + 2] && data_[2 * index + 1] > data_[2 * index + 2]){
